...
fetch_next_desc:
	...
	for (i = 0; i < complq->txq_grp->num_txq; ++i) {
		...
		*cleaned += tx_q->cleaned_pkts;
		...
		else if (tx_q->cleaned_bytes)
			smp_mb();

		if (tx_q->cleaned_pkts && likely(IDPF_DESC_UNUSED(tx_q) > IDPF_TX_WAKE_THRESH)) {
			if (unlikely(netif_tx_queue_stopped(nq)) && !(dont_wake)) {
...
int idpf_tx_maybe_stop_common(struct idpf_queue *tx_q, unsigned int size)
{
	...
	res = 1;
	if (unlikely(IDPF_DESC_UNUSED(tx_q) < size)) {
